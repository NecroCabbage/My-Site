...
namespace OmerBagrutSite.DataModel
{
    public class DBHelper
    {
        ...
        // The Method receive a user objects and insert it to the Database as new row.
        ...
        public int Insert(User user, string table)
        {
            ...
            int numRowsAffected = adapter.Update(dataSet, table);
        ...
        public bool Login(string email, string password)
        {
            ...
            string sql = $"SELECT COUNT(*) FROM UserTbl WHERE Email = '{email}' and Password = '{password}'";
        ...
        public int Update_disconnected(User user, string table)
        {
            ...
            // בניית פקודת SQL
            string SQLStr = $"SELECT * FROM {table} WHERE UserId = {user.UserId}";
            SqlCommand cmd = new SqlCommand(SQLStr, con);
